...
class ReportController extends Controller
{
    ...
    function __construct()
     {
         ...
         $this->middleware('permission:outlet-stock.report', ['only' => ['outlet_stock_report_form']]);
         $this->middleware('permission:outlet-stock.search', ['only' => ['outlet_stock_report_submit']]);
         $this->middleware('permission:warehouse-stock.report', ['only' => ['warehouse_stock_report_form']]);
    ...
    public function warehouse_stock_report_form()
    {
        ...
        return view('admin.report.warehouse_stock_report',compact('title'));
    }
    public function warehouse_stock_report_submit(Request $request)
    {
        ...
        if (Auth::user()->hasRole(['Super Admin', 'Admin'])){
            ...
            if($request->medicine_id == '' || $request->medicine_id == null ){
                ...
                $productSales = OutletStock::where('medicine_id',$request->medicine_id)->where('warehouse_id', '=', $warehouse_id)->where('quantity','>','0')->whereDate('created_at', '>=', $start_date)
                ->whereDate('created_at', '<=', $end_date)->orderBy('id', 'asc')->get();
                $outlet =  Warehouse::where('id',$warehouse_id)->orderby('id','desc')->first('warehouse_name');
                $title = 'All Stock Report '.$outlet->warehouse_name;
            }
          ...
          return view('admin.report.warehouse_stock_report', compact('start_date', 'end_date', 'productSales','title'));
    }



    public function outlet_stock_report_form()
    {
        $title = 'Outlet Stock Report';
        return view('admin.report.outlet_stock_report',compact('title'));
    }
    public function outlet_stock_report_submit(Request $request)
    {
        ...
        if (Auth::user()->hasRole(['Super Admin', 'Admin'])){
          ...
          return view('admin.report.outlet_stock_report', compact('start_date', 'end_date', 'productSales','title'));
    }
